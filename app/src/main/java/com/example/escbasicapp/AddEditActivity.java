...
public class AddEditActivity extends AppCompatActivity {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        String phoneNum = getIntent().getStringExtra("phone_num"); //번호 불러오기
    ...
    private void setUpUI(){
        ...
        phone.getEditText().addTextChangedListener(new PhoneNumberFormattingTextWatcher()); //폰번호 형식으로 바꿈. 에뮬에선 확인 불가능
    }

    @Override //뒤로가기 버튼 처리
    public boolean onOptionsItemSelected(@NonNull MenuItem item) {
        switch (item.getItemId()){
            case android.R.id.home: //뒤로가기 하면 홈으로
                finish();
                break;
            case R.id.menu_save:
